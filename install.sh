#!/bin/bash
# Smart Ultimate Installer v4.0 - rima0222
REPO="https://raw.githubusercontent.com/rima0222/AdminPanel/main"

echo "🧹 پاکسازی و آماده‌سازی سیستم..."
...
echo "📂 دریافت فایل‌های اصلی از مخزن..."
...
# مدیریت هوشمند دیتابیس
if [ -f "/root/users.db.backup" ]; then
    echo "♻️ بک‌آپ شناسایی شد! در حال انتقال و همگام‌سازی کاربران..."
    ...
    sqlite3 /root/users.db "CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY, username TEXT, password TEXT, protocol TEXT, user_email TEXT, limit_login INTEGER, created_at DATE DEFAULT CURRENT_DATE, status TEXT);"
...
echo "✅ نصب با موفقیت انجام شد! پورت: 5000"
